...
import { Download, Plus, Search } from 'lucide-solid'
...
export function AccountsView(props: { company: Company }) {
  ...
  const yearIncome = summaries.reduce((s, m) => s + m.income, 0)
  const yearExpenses = summaries.reduce((s, m) => s + m.expenses, 0)
  const maxNet = Math.max(...summaries.map((m) => Math.abs(m.income - m.expenses)))
