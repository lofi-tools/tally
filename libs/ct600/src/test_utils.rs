#![cfg(test)]
//! Test utilities for the Companies House client.
//!
//! Hosts the test-only Companies House infrastructure: the cached client
//! ([`CachedCompaniesHouseClient`]) and the offline test client
//! ([`TestClient`]) with its hardcoded default fixtures, plus the tests for
//! both.
//!
//! [`TestClient`] serves company profiles from a local cache when available,
//! falling back on hardcoded default fixtures, and finally on the live
//! Companies House API when a live client is configured.  It is auto-built
//! from the environment with sensible defaults, so tests run with zero
...
impl TestClient {
    ...
    /// Fetch the CT600 company header boxes for the tax computation, using
    /// the cache/fixtures/live lookup above.
    pub async fn company_form_values(
        &self,
        tax: &Frs105CorpTax,
    ) -> Result<CompanyFormValues, TestClientErr> {
        let profile = self.get_company_profile(tax.company_number()).await?;
        Ok(CompanyFormValues::from_profile_and_tax(&profile, tax))
    }
...
/// Hardcoded test data: fictional company profiles and sample tax
/// computations served when the local cache misses, so tests run with
...
impl TestData {
    ...
    /// The company profile of the fictional test company (company number
    /// [`Self::default_company_number`]).
    pub fn default_company() -> CompanyProfile {
        CompanyProfile {
            company_number: Self::default_company_number().to_string(),
            company_name: "EXAMPLE CORP LTD".to_string(),
    ...
    /// The hardcoded default fixture: the fictional company profile for
    /// the given company number, or `None` for unknown companies.
    ...
    /// A sample FRS 105 tax computation for the fictional test company,
    /// the same company as the hardcoded default fixture (company number
    ...
    pub fn sample_tax() -> Frs105CorpTax {
        ...
        facts.non_numeric.insert(
            ...
            "EXAMPLE PRODUCTS LTD".to_string(),
        );
        facts
            .non_numeric
        ...
        let default_company = Self::default_company();
        let company = ixbrl::company::Company::new(
            default_company.company_name.as_str(),
            "1234567890",
            default_company.company_number.as_str(),
...
mod tests {
    ...
    /// Endpoint `company_form_values` (company profile + CT600 company
    /// header boxes), served from the hardcoded default fixture via
    /// `TestClient`.
    #[tokio::test]
    async fn company_form_values_12345678_serves_default_fixtures() {
        let client = TestClient::default();
        let tax = TestData::sample_tax();
        ...
        let values = client
            ...
            .await
            .expect("deriving company form values should succeed");

        let company = TestData::default_company();
        assert_eq!(values.company_name, company.company_name);
        assert_eq!(values.company_number, company.company_number);
        assert_eq!(
            values.type_of_company,
            company
                .company_type
    ...
    async fn test_client_serves_from_cache_before_default_fixture() {
        ...
        let profile = CompanyProfile {
            company_number: TestData::default_company_number().to_string(),
            company_name: "CACHED PRODUCTS LTD".to_string(),
            company_status: Some("active".to_string()),
            company_status_detail: None,
            date_of_creation: Some("2001-01-01".to_string()),
            date_of_dissolution: None,
    ...
    async fn test_cached_client_serves_from_cache_without_network() {
        ...
        let profile = CompanyProfile {
            company_number: TestData::default_company_number().to_string(),
            company_name: "EXAMPLE PRODUCTS LTD".to_string(),
            company_status: Some("active".to_string()),
            company_status_detail: None,
            date_of_creation: Some("2001-01-01".to_string()),
            date_of_dissolution: None,
