...
| Filed accounts document (the iXBRL a `download_filing` returns) | `companies_house::parse_filed_accounts(html)` → `FiledBalanceSheet` (period + `PreviousPeriodFigures`): a generic iXBRL pass into the shared fact IR (`reports::ixbrl_fmt`), then an accounts-specific pass resolving the FRC taxonomy facts (both the `core:` and `uk-core:` renderings) in the current-period context |
