...
impl CorporationTaxReturn {
    pub fn from_gnucash(
        ...
        tokio::runtime::Runtime::new()
            .unwrap()
            .block_on(Self::from_gnucash_async(
                book,
                accounting_start,
                accounting_end,
                company_name,
    ...
    pub async fn from_gnucash_async(
        book: &rucash::Book<rucash::SQLiteQuery>,
        accounting_start: chrono::NaiveDate,
        accounting_end: chrono::NaiveDate,
        company_name: &str,
        ...
        let accounts = book.accounts().await.expect("accounts");
        let txns = book.transactions().await.expect("transactions");
        let splits = book.splits().await.expect("splits");
        ...
        let sum_abs = |splits: &[(f64, String)], account: &str| -> f64 {
            sum(splits, account).abs()
        };
        ...
        let pensions_current =
            sum_abs(&period_splits, "Expenses:Emoluments:Employer Pension Contribution");
        ...
        let subscriptions_current =
            sum_abs(&period_splits, "Expenses:VAT Purchases:Subscriptions");
        ...
        let total_costs_current = salaries_current + pensions_current + accountancy_current
            + bank_charges_current + office_current + software_current + subscriptions_current
            + sundries_current + telecoms_current + travel_current;
        ...
        let aia_current =
            round_down(sum_abs(&period_splits, "Assets:Capital Equipment:Computer Equipment"));

        let rnd_enhanced_current =
            round_down(sum_abs(&period_splits, "R&D Enhanced Expenditure:Expenditure:Project Iguana:Staffing Costs"));

        let ct_trading_profits_raw =
            profit_before_tax_current - aia_current - rnd_enhanced_current;
        ...
        let corporation_tax_chargeable_payable = round2(corporation_tax_chargeable + marginal_relief);
        ...
        let tax_expense_current =
            sum_abs(&period_splits, "Equity:Corporation Tax:Corporation Tax");
        ...
        let pensions_prev =
            sum_abs(&prev_splits, "Expenses:Emoluments:Employer Pension Contribution");
        ...
        let total_costs_prev = salaries_prev + pensions_prev + accountancy_prev + bank_charges_prev
            + office_prev + software_prev + subscriptions_prev + sundries_prev + telecoms_prev
            + travel_prev;

        let gross_profit_prev = round_down(ct_turnover_prev);
        let profit_before_tax_prev = gross_profit_prev - total_costs_prev;
        let aia_prev = round_down(sum_abs(&prev_splits, "Assets:Capital Equipment:Computer Equipment"));
        let rnd_enhanced_prev = round_down(sum_abs(&prev_splits, "R&D Enhanced Expenditure:Expenditure:Project Iguana:Staffing Costs"));
        ...
        let exp = [
            ...
            ("subscriptions-costs", subscriptions_current, subscriptions_prev),
        ...
        for (id, cur, prev) in exp {
            expenses_by_fy.insert(
                id.to_string(),
                HashMap::from([(2020, cur), (2019, prev)]),
            );
        }
        ...
        CorporationTaxReturn {
            ...
            tax_expense_by_fy: HashMap::from([(2019, tax_expense_prev), (2020, tax_expense_current)]),
    ...
    fn account_path(
        accounts: &[rucash::model::Account<rucash::SQLiteQuery>],
        acc: &rucash::model::Account<rucash::SQLiteQuery>,
    ) -> String {
    ...
    pub fn to_ixbrl(&self) -> String {
        ...
        self.ix_non_numeric(&mut out, "ct-comp:NameOfProductionSoftware", "ctxt-0", "ixbrl-reporter");
        self.ix_non_numeric(&mut out, "ct-comp:VersionOfProductionSoftware", "ctxt-0", "1.2.1");
        self.ix_non_numeric(&mut out, "ct-comp:CompanyName", "ctxt-0", &self.company_name);
        self.ix_non_numeric(&mut out, "ct-comp:TaxReference", "ctxt-0", &self.tax_reference);
        out.push_str("</ix:hidden>");
        ...
        self.write_context_instant(&mut out, "ctxt-0", &self.company_number, &self.accounting_period_end, Some("ct-comp:BusinessTypeDimension"), Some("ct-comp:Company"));
        self.write_context_duration(&mut out, "ctxt-1", &self.company_number, &self.accounting_period_start, &self.accounting_period_end, Some("ct-comp:BusinessTypeDimension"), Some("ct-comp:Company"));
        self.write_context_duration(&mut out, "ctxt-2", &self.company_number, &self.accounting_period_start, &self.accounting_period_end, Some("ct-comp:BusinessTypeDimension"), Some("ct-comp:ManagementExpenses"));
        self.write_context_duration(&mut out, "ctxt-3", &self.company_number, &self.accounting_period_start, &self.accounting_period_end, Some("ct-comp:BusinessTypeDimension"), Some("ct-comp:Company"));
        self.write_context_duration(&mut out, "ctxt-4", &self.company_number, &self.accounting_period_start, &self.accounting_period_end, Some("ct-comp:BusinessTypeDimension"), Some("ct-comp:Trade"));
        self.write_context_duration(&mut out, "ctxt-5", &self.company_number, &self.accounting_period_start, &self.accounting_period_end, Some("ct-comp:BusinessTypeDimension"), Some("ct-comp:Trade"));
        self.write_context_duration(&mut out, "ctxt-8", &self.company_number, &chrono::NaiveDate::from_ymd_opt(2019, 1, 1).unwrap(), &chrono::NaiveDate::from_ymd_opt(2019, 12, 31).unwrap(), Some("ct-comp:BusinessTypeDimension"), Some("ct-comp:Trade"));
        self.write_context_duration(&mut out, "ctxt-9", &self.company_number, &self.accounting_period_start, &self.accounting_period_end, Some("dpl:DetailedAnalysisDimension"), Some("dpl:Item1"));
        self.write_context_duration(&mut out, "ctxt-10", &self.company_number, &chrono::NaiveDate::from_ymd_opt(2019, 1, 1).unwrap(), &chrono::NaiveDate::from_ymd_opt(2019, 12, 31).unwrap(), Some("dpl:DetailedAnalysisDimension"), Some("dpl:Item1"));
        self.write_context_duration(&mut out, "ctxt-11", &self.company_number, &self.accounting_period_start, &self.accounting_period_end, None, None);
        self.write_context_duration(&mut out, "ctxt-12", &self.company_number, &chrono::NaiveDate::from_ymd_opt(2019, 1, 1).unwrap(), &chrono::NaiveDate::from_ymd_opt(2019, 12, 31).unwrap(), None, None);
        self.write_context_duration(&mut out, "ctxt-13", &self.company_number, &self.accounting_period_start, &self.accounting_period_end, Some("dpl:ExpenseTypeDimension"), Some("dpl:AdministrativeExpenses"));
        self.write_context_duration(&mut out, "ctxt-14", &self.company_number, &chrono::NaiveDate::from_ymd_opt(2019, 1, 1).unwrap(), &chrono::NaiveDate::from_ymd_opt(2019, 12, 31).unwrap(), Some("dpl:ExpenseTypeDimension"), Some("dpl:AdministrativeExpenses"));
        self.write_context_duration(&mut out, "ctxt-15", &self.company_number, &chrono::NaiveDate::from_ymd_opt(2019, 1, 1).unwrap(), &chrono::NaiveDate::from_ymd_opt(2019, 12, 31).unwrap(), Some("ct-comp:BusinessTypeDimension"), Some("ct-comp:ManagementExpenses"));
        self.write_context_duration(&mut out, "ctxt-16", &self.company_number, &chrono::NaiveDate::from_ymd_opt(2019, 1, 1).unwrap(), &chrono::NaiveDate::from_ymd_opt(2019, 12, 31).unwrap(), Some("ct-comp:BusinessTypeDimension"), Some("ct-comp:Company"));
    ...
    fn write_context_instant(&self, out: &mut String, id: &str, scheme_id: &str, date: &chrono::NaiveDate, dim: Option<&str>, val: Option<&str>) {
        ...
        out.push_str(&format!("<xbrli:period><xbrli:instant>{}</xbrli:instant></xbrli:period></xbrli:context>", date));
    }

    fn write_context_duration(&self, out: &mut String, id: &str, scheme_id: &str, start: &chrono::NaiveDate, end: &chrono::NaiveDate, dim: Option<&str>, val: Option<&str>) {
        out.push_str(&format!("<xbrli:context id=\"{}\"><xbrli:entity><xbrli:identifier scheme=\"http://www.companieshouse.gov.uk/\">{}</xbrli:identifier>", id, scheme_id));
        if let Some(d) = dim {
            out.push_str(&format!("<xbrli:segment><xbrldi:explicitMember dimension=\"{}\">{}</xbrldi:explicitMember></xbrli:segment>", d, val.unwrap_or("")));
        }
    ...
    fn write_unit(&self, out: &mut String, unit: &str) {
        out.push_str(&format!("<xbrli:unit id=\"iso4217\"><xbrli:measure>{}</xbrli:measure></xbrli:unit>", unit));
    }

    fn ix_non_numeric(&self, out: &mut String, name: &str, ctx: &str, value: &str) {
        out.push_str(&format!("<ix:nonNumeric name=\"{}\" contextRef=\"{}\">{}</ix:nonNumeric>", name, ctx, value));
    }
    ...
    fn write_corporation_tax_return_page(&self, out: &mut String) {
        out.push_str("<div class=\"page\"><div class=\"page .header\"><h2>Corporation Tax Return</h2></div>");
        ...
        self.ix_non_numeric(out, "ct-comp:StartOfPeriodCoveredByReturn", "ctxt-0", &format_date(&self.return_period_start));
        out.push_str("</span></div></div>");

        out.push_str("<div class=\"row\"><div class=\"cell\"><span class=\"label\">Return period end</span></div><div class=\"cell\"><span class=\"value\">");
        self.ix_non_numeric(out, "ct-comp:EndOfPeriodCoveredByReturn", "ctxt-0", &format_date(&self.return_period_end));
        out.push_str("</span></div></div>");
        ...
        self.ix_non_numeric(out, "ct-comp:PeriodOfAccountStartDate", "ctxt-0", &format_date(&self.accounting_period_start));
        out.push_str("</span></div></div>");
        ...
        self.ix_non_numeric(out, "ct-comp:PeriodOfAccountEndDate", "ctxt-0", &format_date(&self.accounting_period_end));
        out.push_str("</span></div></div>");

        out.push_str("<div class=\"row\"><div class=\"cell\"><span class=\"label\">Partner in a firm</span></div><div class=\"cell\"><span class=\"value\">");
        self.ix_non_numeric(out, "ct-comp:CompanyIsAPartnerInAFirm", "ctxt-1", &self.partner_in_a_firm.to_string());
    ...
    fn write_capital_allowances_page(&self, out: &mut String) {
        ...
        self.ix_non_fraction(out, "ct-comp:MainPoolAnnualInvestmentAllowance", "ctxt-2", "iso4217:GBP", self.annual_investment_allowance);
        out.push_str("</span></div></div></div>");
    }

    fn write_profits_and_gains_page(&self, out: &mut String) {
        out.push_str("<div class=\"page\"><div class=\"page .header\"><h2>Profits and gains</h2></div>");
        let fields: &[(&str, &str, &str, f64)] = &[
            ("Trading profits", "ct-comp:AdjustedTradingProfitOfThisPeriod", "ctxt-3", self.adjusted_trading_profit),
            ("Trading losses brought forward", "ct-comp:TradingLossesBroughtForward", "ctxt-3", self.trading_losses_brought_forward),
            ("Net trading profits", "ct-comp:NetTradingProfits", "ctxt-3", self.net_trading_profits),
            ("Net chargeable gains", "ct-comp:NetChargeableGains", "ctxt-1", self.net_chargeable_gains),
            ("Profits before other deductions and reliefs", "ct-comp:ProfitsBeforeOtherDeductionsAndReliefs", "ctxt-3", self.profits_before_deductions),
            ("Profits before donations and group relief", "ct-comp:ProfitsBeforeChargesAndGroupRelief", "ctxt-3", self.profits_before_charges),
            ("Qualifying donations", "ct-comp:QualifyingDonations", "ctxt-1", self.qualifying_donations),
            ("Group relief claimed", "ct-comp:GroupReliefClaimed", "ctxt-1", self.group_relief),
            ("Group relief for carried forward losses", "ct-comp:GroupReliefClaimedForCarriedForwardLosses", "ctxt-1", self.group_relief_carried_forward),
            ("Profits chargeable to Corporation Tax", "ct-comp:TotalProfitsChargeableToCorporationTax", "ctxt-3", self.profits_chargeable_to_corporation_tax),
        ];
    ...
    fn write_losses_page(&self, out: &mut String) {
        ...
        self.ix_non_fraction(out, "ct-comp:TradingLossesOfThisOrLaterAP", "ctxt-3", "iso4217:GBP", self.losses_of_trades_uk);
        self.ix_non_fraction(out, "ct-comp:LossesFromMiscellaneousTransactions", "ctxt-1", "iso4217:GBP", self.losses_from_miscellaneous);
        out.push_str("</div>");
    }

    fn write_tax_chargeable_page(&self, out: &mut String) {
        out.push_str("<div class=\"page\"><div class=\"page .header\"><h2>Tax chargeable</h2></div>");

        self.ix_non_numeric(out, "ct-comp:FinancialYear1CoveredByTheReturn", "ctxt-1", &self.fy1_year.to_string());
        self.ix_non_numeric(out, "ct-comp:FinancialYear2CoveredByTheReturn", "ctxt-1", &self.fy2_year.to_string());
        self.ix_non_fraction(out, "ct-comp:FY1AmountOfProfitChargeableAtFirstRate", "ctxt-3", "iso4217:GBP", self.fy1_profit);
        self.ix_non_fraction(out, "ct-comp:FY2AmountOfProfitChargeableAtFirstRate", "ctxt-3", "iso4217:GBP", self.fy2_profit);
        self.ix_non_fraction(out, "ct-comp:FY1FirstRateOfTax", "ctxt-1", "iso4217:GBP", self.fy1_rate);
        self.ix_non_fraction(out, "ct-comp:FY2FirstRateOfTax", "ctxt-1", "iso4217:GBP", self.fy2_rate);
        self.ix_non_fraction(out, "ct-comp:FY1TaxAtFirstRate", "ctxt-3", "iso4217:GBP", self.fy1_tax);
        self.ix_non_fraction(out, "ct-comp:FY2TaxAtFirstRate", "ctxt-3", "iso4217:GBP", self.fy2_tax);
        self.ix_non_fraction(out, "ct-comp:CorporationTaxChargeable", "ctxt-3", "iso4217:GBP", self.corporation_tax_chargeable);
        self.ix_non_fraction(out, "ct-comp:MarginalRateReliefForRingFenceTradesPayable", "ctxt-1", "iso4217:GBP", self.marginal_relief);
        self.ix_non_fraction(out, "ct-comp:CorporationTaxChargeablePayable", "ctxt-3", "iso4217:GBP", self.corporation_tax_chargeable_payable);
        self.ix_non_fraction(out, "ct-comp:TotalReliefsAndDeductionsInTermsOfTaxPayable", "ctxt-1", "iso4217:GBP", self.total_reliefs_deductions_tax);
        self.ix_non_fraction(out, "ct-comp:NetCorporationTaxPayable", "ctxt-3", "iso4217:GBP", self.net_corporation_tax_payable);
        self.ix_non_fraction(out, "ct-comp:TaxChargeable", "ctxt-3", "iso4217:GBP", self.tax_chargeable);
        self.ix_non_fraction(out, "ct-comp:TaxPayable", "ctxt-3", "iso4217:GBP", self.tax_payable);
    ...
    fn write_rnd_page(&self, out: &mut String) {
        ...
        self.ix_non_numeric(out, "ct-comp:CompanyIsAPartnerInAFirm", "ctxt-1", &self.is_sme.to_string());
        self.ix_non_numeric(out, "ct-comp:CompanyIsAPartnerInAFirm", "ctxt-1", &self.is_large_company.to_string());
        self.ix_non_fraction(out, "ct-comp:SubsidisedQualifyingExpenditureOnIn-HouseDirectRD", "ctxt-4", "iso4217:GBP", self.rnd_qualifying_expenditure);
        self.ix_non_fraction(out, "ct-comp:AdjustmentsAdditionalDeductionForQualifyingRDExpenditureSME", "ctxt-4", "iso4217:GBP", self.rnd_enhanced_expenditure);
        self.ix_non_fraction(out, "ct-comp:AdjustmentsCreativeProductionCompanyAdjustment", "ctxt-5", "iso4217:GBP", self.creative_enhanced_expenditure);
        self.ix_non_fraction(out, "ct-comp:AdjustmentsAdditionalDeductionForQualifyingRDExpenditureSME", "ctxt-4", "iso4217:GBP", self.rnd_creative_enhanced_total);
        self.ix_non_fraction(out, "ct-comp:AdjustmentsAdditionalDeductionForQualifyingRDExpenditureSME", "ctxt-8", "iso4217:GBP", self.rnd_subcontracted_large);
        out.push_str("</div>");
    }
}
...
fn format_date(d: &chrono::NaiveDate) -> String {
    ...
    format!("{}&#160;{}&#160;{}", day.trim_start_matches('0'), month, year)
}
...
mod tests {
    ...
    async fn test_ct_return_from_example2() {
        let query =
            SQLiteQuery::new("example_data/example2/example2.gnucash").expect("open gnucash");
        let book = Book::new(query).await.expect("create book");
        let ct = CorporationTaxReturn::from_gnucash_async(
            &book,
