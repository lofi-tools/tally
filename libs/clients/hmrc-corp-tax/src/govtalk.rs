...
//!
//! The HMRC GovTalk XML envelope: the message structures ([`GovTalkEnvelope`]
//! and friends), the [`Message`] trait and its concrete message types
//! (submission request / acknowledgement / poll / error / response, delete
//! request / response), and the [`decode_govtalk_message`] decoder factory.
...
pub trait Message {
    fn to_xml(&self) -> Result<String> {
        ...
        let xml = quick_xml::se::to_string(&envelope).map_err(|e| Ct600Error::XmlError {
    ...
    fn to_iso_date(date_str: &str) -> Result<String> {
        let date = NaiveDate::parse_from_str(date_str, "%d %B %Y").map_err(|e| {
            Ct600Error::ConfigError {
...
impl GovTalkSubmissionRequest {
    ...
    pub fn compute_irmark(&self) -> Result<String> {
        ...
        let canonical =
            canonicalize(&xml, C14nMode::Inclusive, None, &[] as &[String]).map_err(|e| {
                Ct600Error::C14nError {
                    message: e.to_string(),
                }
            })?;
...
pub fn decode_govtalk_message(xml: &str) -> Result<GovTalkMessage> {
    let envelope: GovTalkEnvelope =
        quick_xml::de::from_str(xml).map_err(|e| Ct600Error::XmlError {
