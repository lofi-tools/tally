...
> owner. It refines the demo-state UX introduced by
> `docs/spec/first-run-onboarding-spec.md` (§4.2, §5.1): the demo banner is
> more prominent, its copy teaches the path to real data, its visibility now
> follows the *selected* company rather than raw company count, and the demo
...
Three gaps remain in the current demo-state UX:
...
2. **Unclear how to get your own data in.** The copy ("Demo data — add your
...
3. **Wrong visibility rule.** The banner only shows when the user has **zero**
   companies. If a user owns companies and selects `Demo Co Ltd` in the
   picker, only a tiny `Demo` badge marks it — exactly the state where
   "this isn't your data" matters most.

## 2. Goals & non-goals

### Goals
- The demo banner is **noticeably more prominent** (stronger surface color,
  an icon) without becoming a full-saturation wall.
- The banner **teaches the two-step path** to real data: add a company, then
  connect a data source.
- The banner **follows the selected company**: it appears whenever demo data
...
- The demo label is **present wherever demo data is visible**: banner +
  picker badge + a `Demo` chip in every view header.
- The demo company **remains available to explore forever** (badged), so
...
| R1 | When is the banner visible? | **Whenever the demo is selected** — including when the user owns companies and picks the demo. |
| R1 | Labeling beyond the banner | **Add a `Demo` chip to each view header** (next to the page title), plus the existing picker badge. |
...
| R2 | Switch affordance | **"View your company" button in the banner** — a secondary action when the demo is selected and real companies exist. |
...
| R3 | Demo after retirement | **Keep in the picker, badged `Demo`** — stays selectable forever (one-way retirement is dropped). |
...
| R4 | `Demo` chip color | **Brown (brand default)** — default `outline` badge, consistent with the current picker badge. |
...
- `demoSelected` — the current company in the picker is `DEMO_COMPANY_ID`.
...
|---|---|---|---|
| **A — onboarding** | demo selected · no real companies | Variant A (two-step) | `Add company` (opens search dialog) |
| **B — viewing demo** | demo selected · real companies exist | Variant B (switch) | `View <first real company>` (switches selection) |
...
Precedence: **A > B > C** when multiple conditions overlap (e.g. demo selected
...
## 5. Banner redesign (`DemoBanner.tsx`)
...
> `Demo data` (bold) · `1. Add your company → 2. Connect a bank or upload a ledger.`
> Actions: `Add company` (primary).

**B — viewing demo** (real companies exist, demo selected):
> `You're viewing demo data` (bold) · `switch to your own company to see your numbers.`
...
## 6. Demo identity everywhere

### 6.1 View-header chips (new)
- Add a `DemoBadge` component to `components/Shared.tsx`:
  `Badge variant="outline"` (brand-default brown), label `Demo`, `fontSize xs`.
...
- Each of the five views (`Accounts`, `Filings`, `Payroll`, `Integrations`,
  ...
  — import `DEMO_COMPANY_ID` from `../mock_data`.
- Effect: whenever demo data is on screen, the page title carries the
  `Demo` label — a second marker alongside the (undismissable) banner.

### 6.2 Picker (unchanged + never retires)
- The demo stays in the picker **forever**, badged `Demo` (outline, brown)
  in both the trigger summary and the item row.
- The `demoRetired` memo and one-way retirement rule in `App.tsx` are
  **removed**: `allCompanies` always includes the demo first.
...
- The zero-companies-of-any-kind screen (onboarding spec §4.4) keeps its own
  hero. With the demo permanent it is unreachable in practice — still kept
  as a fallback.
...
inputs (`demoSelected`, `hasRealCompanies`, `anyDataConnected`). It renders
...
- **Offline banner** (amber solid) stacks above the demo banner when both
...
- `components/DemoBanner.tsx` — becomes one component with a
  `variant: 'onboarding' | 'viewing-demo' | 'empty-data'` prop plus
  `onAddCompany` / `onViewCompany` / `onConnectBank` / `onDismiss`. Copy,
  icon and action labels switch on the variant.
- `App.tsx` — replace `bannerVisible` / `bannerDismissed` / `demoRetired`
...
6. Every view header shows the `Demo` chip whenever demo data is on
   screen; the picker keeps its `Demo` badge.
7. The demo remains selectable in the picker after data connects.
...
- Distinguishing "sample" vs "demo" branding — resolved: fixed on `Demo`.
