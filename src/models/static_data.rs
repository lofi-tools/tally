...
pub static DB: LazyLock<StaticDb> = LazyLock::new(|| StaticDb {
    accounts: AccountsMap::default()
        .with_account(Account { id: AccountId(Cow::Borrowed("BANK")), code: 1200, name: "Company bank account",details: "Starling bank",account_tag: AccountTag::Cash,asset_id: GBP.clone(),loan_apy: None, })
        // Director's loan is an account receivable (debt owed by others to the company) -> it's an asset account
        .with_account(Account { id: AccountId(Cow::Borrowed("DIRECTORS_LOAN")), code: 2301, name: "Director's loan account",details: "Director 1: Nicolas Marshall",account_tag: AccountTag::AccountsReceivable,asset_id: GBP.clone(),loan_apy: Decimal::from_f64_retain(2.0), })
        .with_account(Account { id: AccountId(Cow::Borrowed("SALES")), code: 4010, name: "Sales - Services",details: "",account_tag: AccountTag::Sales,asset_id: GBP.clone(),loan_apy: None, })
        // CLIENTS replaces SALES, but amount is negative. total(SALES) is now derived from -total(CLIENTS)
        .with_account(Account { id: AccountId(Cow::Borrowed("CLIENTS")), code: 4040, name: "Sales - Services",details: "",account_tag: AccountTag::Sales,asset_id: GBP.clone(),loan_apy: None, })
        .with_account(Account { id: AccountId(Cow::Borrowed("NEXO_GBP")), code: 1201, name: "Company spot account - Nexo - GBP",details: "",account_tag: AccountTag::Cash,asset_id: GBP.clone(),loan_apy: None, })
        .with_account(Account { id: AccountId(Cow::Borrowed("NEXO_EUR")), code: 1202, name: "Company spot account - Nexo - EUR",details: "",account_tag: AccountTag::Cash,asset_id: EUR.clone(),loan_apy: None, })
        ...
        // test-only accounts
        .with_account(Account { id: AccountId(Cow::Borrowed("TEST_COMPANY_BORROWS_GBP")), code: 1234, name: "Company borrows (GBP)",details: "",account_tag: AccountTag::LoansPayable,asset_id: GBP.clone(),loan_apy: None, }),
    assets: AllAssets::default()
        .with_asset(Asset { id: GBP.clone() })
        .with_asset(Asset { id: EUR.clone() }),
});

pub static EUR: AssetId = AssetId(Cow::Borrowed("EUR"));
pub static GBP: AssetId = AssetId(Cow::Borrowed("GBP"));
...
pub static BORROW: LazyLock<&'static Account> = LazyLock::new(|| {
    DB.accounts
        .try_get(&AccountId(Cow::Borrowed("TEST_COMPANY_BORROWS_GBP")))
        .unwrap()
});
